...
*/
contract coinCore is Ownable{
        //@param NAME is defines the name of the token as ERC721 standared 
		string public constant NAME = "Lovecoincore";
         ...
         */
		function getLoveinfo(uint256 _coinID) external view returns(address _owner,bytes32 _data) {
        ...
        // getLoveinfo return the coinOwner address and the data embedded in coin 
			return(lovecoin[_coinID].coinOwner,lovecoin[_coinID].data);
		}
		 ...
		 */ 
		function giftLove(uint256 _coinID, address _to, bytes32 _data) public returns (bool) { 
		...
		function totalredeemcount() public constant returns(uint256)  {
				return redeemCount;
		}
